        ...
        <ul class="nav nav-pills custom-tab-nav mb-4" id="pills-tab" role="tablist">
            <li class="nav-item" role="presentation">
                <button class="nav-link active" id="pills-popular-tab" data-bs-toggle="pill"
                        ...
                        aria-selected="true">Популярні
                </button>
            </li>
            <li class="nav-item" role="presentation">
                <button class="nav-link" id="pills-latest-tab" data-bs-toggle="pill" data-bs-target="#pills-latest"
                        type="button" role="tab" aria-controls="pills-latest" aria-selected="false">Останні
                </button>
            </li>
        </ul>
            ...
            <div class="tab-pane fade show active" id="pills-popular" role="tabpanel"
                ...
                @foreach($popular as $article)
                    <div class="post-entry-1 border-bottom">
                        ...
                        <h2 class="mb-2"><a href="{{ route('show.article', $article->id) }}">{{ $article->title }}</a>
                        </h2>
                ...
                @foreach($latest as $article)
                    <div class="post-entry-1 border-bottom">
                        <div class="post-meta">
                            ...
                            @endforeach
                            <span class="mx-1">&bullet;</span>
                            <span>{{ \Carbon\Carbon::parse($article->created_at)->diffForHumans() }}</span></div>
                        <h2 class="mb-2"><a href="{{ route('show.article', $article->id) }}">{{ $article->title }}</a>
                        </h2>
                        <span class="author mb-3 d-block">{{ $article->user?->name }}</span>
                    </div>
